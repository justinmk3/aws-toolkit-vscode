...
import apiConfig = require('../../../types/codeaws-2020-12-01.json')
...
export function getCawsConfig(): CawsConfig {
    ...
    if (stage === 'gamma') {
        return {
            region: 'us-west-2',
            endpoint: 'https://public.api-gamma.quokka.codes',
            hostname: 'integ.stage.quokka.codes',
            gitHostname: 'git.gamma.source.caws.dev-tools.aws.dev',
        }
    } else {
        return {
            region: 'us-east-1',
            endpoint: 'https://public.api.quokka.codes',
...
class CawsClientInternal {
    ...
    public get identity(): ConnectedCawsClient['identity'] {
        if (!this.userDetails) {
            throw new Error('quokka.codes client is not connected')
        }
    ...
    private async call<T>(req: AWS.Request<T, AWS.AWSError>, silent: boolean, defaultVal?: T): Promise<T> {
        ...
        if (!this.bearerTokenProvider) {
            throw new ToolkitError('quokka.codes client is not logged-in', { code: 'NotLoggedIn' })
        }
    ...
    public async verifySession(): Promise<UserDetails> {
        ...
        if (this.userId && this.userId !== resp.identity) {
            throw new Error('quokka.codes identity does not match the one provided by the client')
        }
    ...
    private async getUserDetails(args: caws.GetUserDetailsRequest) {
        ...
        if (resp.version !== '1') {
            throw new Error(`quokka.codes 'getUserDetails' API returned an unsupported version: ${resp.version}`)
        }
    ...
    public async startWorkspaceWithProgress(
        ...
        const progress = await showMessageWithCancel(localize('AWS.caws.startMde.message', 'quokka.codes'), timeout)
