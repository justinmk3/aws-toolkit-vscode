...
export async function ensureDependencies(
    ...
    if (tools.isErr()) {
        ...
        tools.err().forEach(d => {
            if (d.reason) {
                getLogger().error(`quokka.codes: failed to get tool "${d.name}": ${d.reason}`)
            }
        })
    ...
    if (config.isErr()) {
        ...
        getLogger().error(`quokka.codes: failed to add ssh config section: ${err.message}`)
...
async function verifySSHHost({
    ...
    if (!hasProxyCommand) {
        if (configSection !== undefined) {
            getLogger().warn(
                `quokka.codes: SSH config: found old/outdated "${configHostName}" section:\n%O`,
                configSection
            )
