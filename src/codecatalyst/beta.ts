...
const VSIX_LOCATION = 'https://d841teaiyqu4i.cloudfront.net/aws-toolkit-vscode.vsix'
...
async function promptInstallToolkit(pluginPath: vscode.Uri, newVersion: string): Promise<void> {
    ...
    const response = await vscode.window.showInformationMessage(
        localize(
            ...
            `New version of AWS Toolkit is available at the CodeCatalyst beta URL (https://d841teaiyqu4i.cloudfront.net/aws-toolkit-vscode.vsix). Install the new version "{0}" to continue using CodeCatalyst.`,
            newVersion
        ),
        installBtn
    )
