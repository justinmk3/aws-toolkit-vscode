...
export async function autoConnect(authProvider: CawsAuthenticationProvider) {
    for (const account of authProvider.listAccounts().filter(({ metadata }) => metadata.canAutoConnect)) {
        getLogger().info(`quokka.codes: trying to auto-connect with user: ${account.label}`)

        try {
            const creds = await authProvider.createSession(account)
            getLogger().info(`quokka.codes: auto-connected with user: ${account.label}`)

            return creds
        } catch (err) {
            getLogger().debug(`quokka.codes: unable to auto-connect with user "${account.label}": %O`, err)
        }
    }
}
...
export async function openDevelopmentWorkspace(
    ...
    const SessionProcess = createBoundProcess(cawsEnvProvider).extend({
        onStdout(stdout) {
            getLogger().verbose(`quokka.codes connect: ${workspace.id}: ${stdout}`)
        },
        onStderr(stderr) {
            getLogger().verbose(`quokka.codes connect: ${workspace.id}: ${stderr}`)
        },
