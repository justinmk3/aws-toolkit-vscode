...
export async function login(authProvider: CawsAuthenticationProvider, client: CawsClient): Promise<LoginResult> {
    ...
    try {
        ...
        getLogger().error('quokka.codes: failed to login: %O', err)
        return 'Failed'
    }
}
    ...
    try {
        ...
        showViewLogsMessage(
            localize(
                ...
                'Failed to start quokka.codes development workspace "{0}": {1}',
                resource.id,
...
function createClientInjector(
    ...
    return async (command, ...args) => {
        ...
        if (!client.connected) {
            ...
            if (result === 'Failed') {
                globals.window.showErrorMessage('AWS: Not connected to quokka.codes')
            }
